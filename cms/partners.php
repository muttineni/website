...
if (is_numeric($form_fields['state_id']) and $form_fields['state_id'] > 0) $where_clause .= ' AND state_id = '.$form_fields['state_id'];
if (is_numeric($form_fields['country_id']) and $form_fields['country_id'] > 0) $where_clause .= ' AND country_id = '.$form_fields['country_id'];
if (is_numeric($form_fields['is_partner']) and $form_fields['is_partner'] != -1) $where_clause .= ' AND is_partner = '.$form_fields['is_partner'];
else $form_fields['is_partner'] = -1;
if (is_numeric($form_fields['is_diocese']) and $form_fields['is_diocese'] != -1) $where_clause .= ' AND is_diocese = '.$form_fields['is_diocese'];
else $form_fields['is_diocese'] = -1;
foreach ($form_fields as $key => $value)
{
  if (is_string($value)) $form_fields[$key] = stripslashes($value);
}
/* query sorting parameters */
$sort_field_array = array(1 => 'partner_id', 'description', 'is_partner', 'is_diocese');
$sort_options_array = array(1 => 'Partner ID', 'Description', 'Partner', 'Diocese');
...
if (isset($form_fields['export_data']) and isset($form_fields['export_format']))
{
  ...
  $export_query = <<< END_QUERY
    SELECT partner_id, description, IF(is_partner, 'Yes', 'No') AS is_partner, IF(is_diocese, 'Yes', 'No') AS is_diocese
    FROM partners
    WHERE 1
END_QUERY;
  ...
  $export_array = array();
  $export_array[] = array('Partner ID', 'Description', 'Partner', 'Diocese');
  while ($record = mysql_fetch_row($result)) $export_array[] = $record;
  switch ($form_fields['export_format'])
  {
    case 1:
      vlc_export_data($export_array, 'partners', 1);
      break;
    case 2:
      vlc_export_data($export_array, 'partners', 2, 'P');
      break;
    case 3:
      vlc_export_data($export_array, 'partners', 2, 'L');
      break;
  }
}
/* url parameters */
...
{
  $first_url = 'cms/partners.php?'.$url_parameters.'&start='.$first_start;
  ...
  $prev_url = 'cms/partners.php?'.$url_parameters.'&start='.$prev_start;
...
{
  $next_url = 'cms/partners.php?'.$url_parameters.'&start='.$next_start;
  ...
  $last_url = 'cms/partners.php?'.$url_parameters.'&start='.$last_start;
...
if ($total_rows > 0)
{
  ...
  foreach ($page_nav_array as $page_num => $page_start)
  {
    ...
    {
      $page_link_url = 'cms/partners.php?'.$url_parameters.'&start='.$page_start;
...
/* yes/no array */
$yes_no_options_array = array(1 => 'Yes', 0 => 'No');
...
$output = '';
$output .= '<p align="center">'.vlc_internal_link('Click here to create a new partner.', 'cms/partner_details.php').'</p>';
$output .= '<form method="get" action="partners.php">';
...
$output .= '<tr><td align="right"><nobr><b>Partner ID:</b></nobr></td><td colspan="3"><div class="yui-skin-sam"><div id="ac_partner_id" class="autocomplete"><input type="text" name="partner_id" id="ac_partner_id_field" value="'.$form_fields['partner_id'].'"><div id="ac_partner_id_container"></div></div></div></td></tr>';
$output .= '<tr><td align="right"><nobr><b>Description:</b></nobr></td><td colspan="3"><div class="yui-skin-sam"><div id="ac_description" class="autocomplete"><input type="text" name="description" id="ac_description_field" value="'.$form_fields['description'].'"><div id="ac_description_container"></div></div></div></td></tr>';
$output .= '<tr>';
$output .= '<td align="right"><nobr><b>Partner:</b></nobr></td><td>'.vlc_select_box($yes_no_options_array, 'array', 'is_partner', $form_fields['is_partner'], false).'</td>';
$output .= '<td align="right"><nobr><b>Diocese:</b></nobr></td><td>'.vlc_select_box($yes_no_options_array, 'array', 'is_diocese', $form_fields['is_diocese'], false).'</td>';
$output .= '</tr>';
$output .= '<tr>';
...
$partner_query = <<< END_QUERY
  SELECT partner_id, description, is_partner, is_diocese
  FROM partners
  WHERE 1
  $where_clause
END_QUERY;
$partner_query .= $order_clause;
$partner_query .= $limit_clause;
...
if (mysql_num_rows($result) > 0)
{
  ...
  {
    $sort_asc_url = 'cms/partners.php?'.$url_parameters_without_sort.'sort_by[]='.$sort_by_key.'&sort_dir[]=1&sort_by[]=NULL&sort_dir[]=NULL&sort_by[]=NULL&sort_dir[]=NULL&num='.$form_fields['num'].'&start=0';
    $sort_asc_link = vlc_internal_link('<img border="0" src="'.$site_info['images_url'].'sort-asc.png">', $sort_asc_url);
    $sort_desc_url = 'cms/partners.php?'.$url_parameters_without_sort.'sort_by[]='.$sort_by_key.'&sort_dir[]=2&sort_by[]=NULL&sort_dir[]=NULL&sort_by[]=NULL&sort_dir[]=NULL&num='.$form_fields['num'].'&start=0';
  ...
  while ($record = mysql_fetch_array($result))
  {
    ...
    $output .= '<td align="center">'.vlc_internal_link($record['partner_id'], 'cms/partner_details.php?partner='.$record['partner_id']).'</td>';
    $output .= '<td>'.$record['description'].'</td>';
    $output .= '<td align="center">'.$yes_no_options_array[$record['is_partner']].'</td>';
    $output .= '<td align="center">'.$yes_no_options_array[$record['is_diocese']].'</td>';
    $output .= '</tr>';
    $result_num++;
  }
  $output .= '</table>';
...
YAHOO.example.ACFlatData = new function() {
    ...
    /* partner_id */
    this.ac_partner_id_data_source = new YAHOO.widget.DS_XHR('xhr.php', ["\n", "\t"]);
    this.ac_partner_id_data_source.responseType = YAHOO.widget.DS_XHR.TYPE_FLAT;
    this.ac_partner_id_data_source.scriptQueryAppend = 'field=partner_id';
    this.ac_partner_id_widget = new YAHOO.widget.AutoComplete('ac_partner_id_field', 'ac_partner_id_container', this.ac_partner_id_data_source);
    this.ac_partner_id_widget.autoHighlight = false;
    this.ac_partner_id_widget.formatResult = function(result_array, query_string) {
    ...
    this.ac_description_data_source.scriptQueryAppend = 'field=partner_desc';
