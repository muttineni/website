...
END_QUERY;
  ...
  if (!ereg('^[a-zA-Z0-9_]{6}$', $form_fields['username'])) $error_message .= '<li>'.$lang['profile']['profile']['status']['username-guidelines'].'</li>';
  /* is password 6 chars and only consists of letters, numbers, and underscore? */
  if (!ereg('^[a-zA-Z0-9_]{6}$', $form_fields['password'])) $error_message .= '<li>'.$lang['profile']['profile']['status']['password-guidelines'].'</li>';
  /* do passwords match? */
...
if (!ereg('^[^@]+@[^@]+\.[^@]+$', $form_fields['primary_email'])) $error_message .= '<li>'.$lang['profile']['profile']['status']['email-invalid'].'</li>';

/* if errors have occurred, go back to form */
if (strlen($error_message))
{
  $_SESSION['form_fields'] = $form_fields;
  vlc_exit_page($error_message, 'error', 'profile/ispd_profile.php');
}
...
END_UPDATE;
  ...
  vlc_exit_page($lang['profile']['profile']['status']['update-profile-success'], 'success', 'profile/profile.php');
}
?>
