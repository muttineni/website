...
if(isset($_FILES['Lillies']['tmp_name']))
{
	echo $_FILES['Lillies']['tmp_name'];
	
	$ch = curl_init();
	
	$cfile = new CURLfile($_FILES['Lillies']['tmp_name'], $_FILES['Lillies']['type'],$_FILES['Lillies']['name']);
	$data = array("myLillies"=>$cfile);
	
	curl_setopt($ch, CURLOPT_URL,"http://localhost/public_html_test/profile/upload_image.php");
