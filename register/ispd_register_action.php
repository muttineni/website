...
if (isset($form_fields['register']))
{
  if ($form_fields['registration_type_id'] == 1)
  {
    ...
    $form_fields['previous_discount_type_id'] = '';
    $form_fields['discount_type_id'] = 4;
    $form_fields['partner_id'] = -1;
  }
...
elseif (isset($form_fields['cancel']))
{
  ...
  vlc_exit_page($lang['register']['status']['cancel-message'], 'success', 'profile/');
}
...
switch ($form_to_show)
{
  ...
  case 1:
    ...
    break;
  case 2:
    $redirect_to = 'register/ispd_index.php';
    break;
  /* diocese form */
  case 3:
...
END_QUERY;
    ...
    /* confirm form */
    if ($form_to_show == 4) $redirect_to = 'register/ispd_register_confirm.php';
    /* success form */
    else
    {
      /* check for errors */
...
END_QUERY;
      ...
      if ($form_fields['action_id'] == 1 and $num_students >= 300) $error_message = '<li>'.$lang['register']['status']['course-full'].'</li>';
      /* cannot put on waiting list because course is not full */
      if ($form_fields['action_id'] == 2 and $num_students < 300) $error_message = '<li>'.$lang['register']['status']['course-not-full'].'</li>';
...
END_QUERY;
      ...
      if (strlen($error_message)) vlc_exit_page($error_message, 'error', 'register/ispd_index.php');
      ...
      $course_details['course_info_link'] = 'https://vlcff.udayton.edu/course_info/' . $course_details['code'] . '.html';
      ...
      if ($form_fields['action_id'] == 1)
      {
        ...
        if ($form_fields['registration_type_id'] == 1)
        {
          ...
          $course_details_message = sprintf($lang['register']['email']['register-details']['ISPD-message'], $user_info['name'], utf8_encode($course_details['description']), $course_details['code'], $course_details['cycle_start'], $facilitator_name, $course_details['course_info_link'], $user_info['username'], $user_info['email']);
        }
        else
        {
          $course_details_subject = $lang['register']['email']['register-details']['subject'].' ('.$lang['register']['common']['misc']['undergraduate-credit'].')';
          $course_details_message = sprintf($lang['register']['email']['register-details']['ISPD-message'], $course_details['email_date'], $user_info['name'], utf8_encode($course_details['description']), $course_details['code'], $lang['register']['email']['register-details']['credit-invoice-notice'], $lang['register']['email']['register-details']['start-video'], $course_details['cycle_start'], $facilitator_name, $course_materials_email, $site_info['vlcff_email'], $user_info['username'], $user_info['email']);
        }
      }
      /* action = waiting list */
...
END_QUERY;
      ...
      $from_email = $site_info['support_email'];
      $reply_email = $site_info['support_email'];
      ...
      if (isset($subject)) vlc_utf8_mail($to, $subject, $message, $from_name, $from_email, $reply_email);
      if (isset($course_details_subject)) vlc_utf8_mail($to, $course_details_subject, $course_details_message, $from_name, $from_email, $reply_email);
      $redirect_to = 'register/ispd_register_success.php';
    }
